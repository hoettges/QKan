...
class ExportTask:
    ...
    def exportKanaldaten(
       ...
       #     return
        ...
        FROM
            einleit AS e"""+auswahlw+""" GROUP BY e.schnam"""
        ...
        sql = """SELECT
            h.haltnam AS name, h.schoben AS schoben, h.schunten AS schunten, h.laenge, h.ks, h.haltungstyp, h.hoehe, h.breite
        FROM
            haltungen AS h
        JOIN
            teilgebiete AS g
        ...
        sql = """SELECT
                    h.haltnam AS name, h.schoben AS schoben, h.schunten AS schunten, h.sohleunten
                FROM
                    haltungen AS h
                JOIN
                    teilgebiete AS g
                ON 
                    Intersects(h.geom,g.geom) and h.haltungstyp IS 'Pumpe'
                GROUP BY h.haltnam"""
        ...
        sql = """SELECT
                        ...
                        JOIN
                            teilgebiete AS g
                        ON 
                            Intersects(h.geom,g.geom) and h.haltungstyp IS 'Wehr'
                        GROUP BY h.haltnam"""
        ...
        sql = """SELECT
            min(X(s.geop)) AS xmin, min(y(s.geop)) AS ymin, max(X(s.geop)) AS xmax, max(y(s.geop)) AS ymax
        FROM
            schaechte AS s
        JOIN
            teilgebiete AS g
        ON 
            Intersects(s.geop,g.geom)"""
        self.dbQK.sql(sql)

        # (entfaellt)

        b = self.dbQK.fetchone()

        # In allen Feldern None durch NULL ersetzen
        c = [0 if el is None else el for el in b]

        data = "{:<10.3f} {:<10.3f} {:<10.3f} {:<10.3f}\n".format(
            math.floor((float(c[0]) - 200.0) / 200.0) * 200.0,
            math.floor((float(c[1]) - 200.0) / 200.0) * 200.0,
            math.ceil((float(c[2]) + 200.0) / 200.0) * 200.0,
            math.ceil((float(c[3]) + 200.0) / 200.0) * 200.0,
        )
